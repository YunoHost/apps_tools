...
def cleanup():
    ...
    for s in [
        ...
        content = re.sub(
            r'(ynh_script_progression\s*\S*"(Install|Updat|Upgrad|Reinstall|Remov)\S* (node|ruby|go)\S*\.\.\.")',
            r"#REMOVEME? \1",
            content,
            flags=re.IGNORECASE,
        )
    ...
    if os.path.exists("scripts/_common.sh"):
        ...
        common_is_empty = not any(
            l for l in _common_sh_content if l.strip() and not l.strip().startswith("#")
        )
        if common_is_empty:
